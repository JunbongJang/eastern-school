...
export class ServerService {
  ...
  checkLoggedInFromServer() {
    return this.httpClient.get<string>('/academy/server/check_logged_in.php')
      .pipe(
        retry(2), // retry a failed request up to 3 times
        catchError(this.handleError) // then handle the error
      );
  ...
  getUserFromServer(member_id: string, learn_gubun: string) {
    return this.httpClient.get<string>('/academy/server/academy_user.php?member_id=' + member_id + '&learn_gubun=' + learn_gubun)
      .pipe(
        retry(2), // retry a failed request up to 3 times
        catchError(this.handleError) // then handle the error
      );
  ...
  getJindoFromServer(member_id: string, learn_gubun: string) {
    return this.httpClient.get<string>('/academy/server/academy_jindo.php?member_id=' + member_id + '&learn_gubun=' + learn_gubun)
      .pipe(
        retry(2), // retry a failed request up to 3 times
        catchError(this.handleError) // then handle the error
      );
  ...
  getStudySetupFromServer(gubun: string, level: string, chapter: string) {
    return this.httpClient.get<string>('/academy/server/academy_study_setup.php?learn_gubun=' + gubun + '&lev=' + level + '&chap=' + chapter)
      .pipe(
        retry(2), // retry a failed request up to 3 times
        catchError(this.handleError) // then handle the error
      );
  ...
  getLevelSetupFromServer(gubun: string) {
    if (gubun === this.viewStateService.STUDY_FINAL) {
      gubun = 'final';
    }
    return this.httpClient.get<string>('/academy/server/academy_level_setup.php?learn_gubun=' + gubun)
      .pipe(
        retry(2), // retry a failed request up to 3 times
        catchError(this.handleError) // then handle the error
      );
